window.onload = function () {
    ...
    const map = new ol.Map({
        ...
        view: new ol.View({
            center: ol.proj.fromLonLat([79.529158, 17.983326]),
            zoom: 18
        })
    });
    ...
    let allFeatures = []; // Global for export
...
.then(data => {
    ...
    function applyFilters() {
        ...
        const filtered = allFeatures.filter(f => {
            ...
            const matchDate = (!fromDate.value || featureTime >= from) &&
                              (!toDate.value || featureTime <= to);
